* Boundary conditions in Kynema-SGF

This file describes the various boundary conditions available in =kynema-sgf= and
...
=kynema-sgf= defines several boundary conditions that the user can specify in the input
